...
function TeamDetail({ team, onBack }: { team: TeamStats; onBack: () => void }) {
  const winRate = ((team.won / team.played) * 100).toFixed(1);
  const goalAvg = (team.goalsFor / team.played).toFixed(2);
  ...
  return (
    ...
    >
            ...
            <div className="flex items-center gap-2 mt-1">
              {team.titles > 0 ? (
                <span className="text-sca-gold text-sm font-bold">
                  {Array.from({ length: team.titles }).map(() => "🏆").join("")} {team.titles}x Campeón Mundial
                </span>
              ) : (
      ...
      <div className="rounded-xl border border-slate-700/50 bg-sca-surface/60 p-4 space-y-4">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Estadísticas de Copa del Mundo</h3>
        ...
        <div className="flex justify-between items-center py-1">
          <span className="text-xs text-slate-400">Primer participación</span>
...
export default function CentralDeDatos() {
  ...
  const [filter, setFilter] = useState<"all" | "CONMEBOL" | "UEFA" | "CONCACAF" | "CAF" | "AFC">("all");
        ...
        {selectedTeam ? (
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">Historia del Mundial 1930 — 2022</p>
            </div>
            ...
            <div className="rounded-xl border border-slate-700/50 bg-sca-surface/60 overflow-hidden">
              ...
              >
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                  🏆 Ediciones del Mundial (1930–2022)
                </span>
                    ...
                    <div className="px-4 pb-4 space-y-1 max-h-64 overflow-y-auto">
                      {worldCupEditions.map((e) => (
                        <div key={e.year} className="flex items-center gap-2 text-xs py-1.5 border-b border-slate-800 last:border-0">
                          <span className="text-sca-gold font-bold w-10">{e.year}</span>
                          <span className="text-slate-500 w-24 truncate">{e.host}</span>
                          <span className="text-white font-semibold flex-1">🏆 {e.champion}</span>
                          <span className="text-slate-500">{e.runnerUp}</span>
                        </div>
                      ))}
                    </div>
                  </motion.div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              {(["all", "CONMEBOL", "UEFA", "CONCACAF", "CAF", "AFC"] as const).map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
